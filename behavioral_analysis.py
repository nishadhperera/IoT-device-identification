...
def calc_IA_features(packet_list, filter_con):
    ...
    plot_list(IA_times_list[0], "Inter arrival time variation with the packet count (%s traffic)" % filter_con, "Packet count",
...
def calc_periodic_statistics(packet_list, period_in_minutes, filter_con):
    global prev_packet
    start_time = 0
    time_period = period_in_minutes * 60
    IA_array = []
    ...
    for i, (packet, dev_name) in enumerate(packet_list):
        ...
        if prev_packet == "":
            ...
            if (packet.time - start_time) <= time_period:
...
def calc_protocol_freq(packet_list, period_in_minutes, filter_con):
    ...
    for i, (packet, dev_name) in enumerate(packet_list):
        ...
        if (packet.time - start_time) <= time_period:
            ...
            http_counter = http_counter + fe.get_http_feature(packet, tl_pro)
            bootp_val, dhcp_val = fe.get_bootp_dhcp_feature(packet, tl_pro)
            dhcp_counter = dhcp_counter + dhcp_val
            dns_counter = dns_counter + fe.get_dns_feature(packet, tl_pro)
        else:
...
def load_behavior_features(folder):
    ...
    piped_to_end_generator = calc_protocol_freq(piped_to_prot_freq, sample_time, filter)
