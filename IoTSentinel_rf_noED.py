...
try:
    ...
    print("Pickling successful IoTSentinel_random_forest with no edit distance ......")
...
X_train, X_test, y_train, y_test = train_test_split(dataset_X, dataset_y, test_size=0, random_state=0)
...
for iter in range(num_of_iter):
    ...
    for f in range(X_train.shape[1]):
        if indices[f]%23 not in f_importance:
            f_importance[indices[f] % 23] = importances[indices[f]]
        else:
            f_importance[indices[f] % 23] += importances[indices[f]]
